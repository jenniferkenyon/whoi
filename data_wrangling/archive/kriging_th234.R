...
i = 1
for (i in 1:dim(th234_pacific)[1]) { # loop over all data in dataset
  if (is.na(th234_pacific$`Total_234Th_dpm/L`[i]) == TRUE) { # if total thorium doesnt exist
    if (is.na(th234_pacific$`Part_234Th_Small_dpm/L`[i]) == FALSE) { # and small particulate thorium exists
      if (is.na(th234_pacific$`Part_234Th_Large_dpm/L`[i]) == FALSE) { # and large particulate thorium exists
        if (is.na(th234_pacific$`Diss_234Th_dpm/L`[i]) == FALSE) { # and dissolved thorium exists
          th234_pacific$`Total_234Th_dpm/L`[i] <- th234_pacific$`Part_234Th_Small_dpm/L`[i] + # add togther allthorium
                                                  th234_pacific$`Part_234Th_Large_dpm/L`[i] +
                                                  th234_pacific$`Diss_234Th_dpm/L`[i]
        }
      }
    }
  }
...
# Remove NA from data ---------------------------------------------------------
th234_pacific <- th234_pacific[-which(is.na(th234_pacific$Latitude)), ] # latitude
th234_pacific <- th234_pacific[-which(is.na(th234_pacific$Longitude)), ] # longitude
th234_pacific <- th234_pacific[-which(is.na(th234_pacific$Depth_m)), ] # depth of each measurement
th234_pacific <- th234_pacific[-which(is.na(th234_pacific$`Total_234Th_dpm/L`)), ] # total Th234
th234_pacific <- th234_pacific[-which(th234_pacific$`Total_234Th_dpm/L` > 10), ] # th234_tot that are too big

# Remove China and Japanese seas data:
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "South China Sea"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "Funka Bay"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "Funka Bay of Hokkaido, Japan"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "Southern South China Sea"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "oligotrophic northern South China Sea"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "main steam of the Kuroshio Current"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "southern South China Sea"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Project_Name == "JGOFS-South East Asia Time-Series Station (SEATS)"), ]
#th234_pacific <- th234_pacific[-which(th234_pacific$Project_Name == "Carbon Cycling in the China Seas: Budget, Controls and Ocean Acidification (CHOICE-C)"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "Ulleung Basin of the East/Japan Sea"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Region == "Western North Pacific"), ]
#th234_pacific <- th234_pacific[-which(th234_pacific$Region == "Central North Pacific: Manzanillo (Mexico) and Hawaii"), ]
th234_pacific <- th234_pacific[-which(th234_pacific$Cruise_ID_1 == "MR05-04"), ]

# Make data for total original locations:
th234_locations_total <- th234_data_ceballos[-which(is.na(th234_data_ceballos$Latitude)), ] # get rid of NA in latitude
th234_locations_total <- th234_data_ceballos[-which(is.na(th234_data_ceballos$Longitude)), ] # get rid of NA in longitude
th234_locations_total <- th234_locations_total[-which(abs(th234_locations_total$Latitude) > 90), ] # proper latitude
th234_locations_total <- th234_locations_total[-which(abs(th234_locations_total$Longitude) > 180), ] # proper longitude

# Make data frames:
th234_locations_old1 <- data.frame(LAT = th234_locations_total$Latitude, 
                                  LON = th234_locations_total$Longitude)
th234_locations_new1 <- data.frame(LAT = th234_pacific$Latitude, 
                                  LON = th234_pacific$Longitude)
th234_locations_new2 <- data.frame(LAT = th234_data_exports$Latitude, 
                                   LON = th234_data_exports$Longitude)
th234_locations_new <- rbind(th234_locations_new1, th234_locations_new2)
th234_locations_old <- rbind(th234_locations_old1, th234_locations_new2)
